...
LINEEND=" \\"
COUNT=`wc -l ../nx505j/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../nx505j/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../nx505j/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
